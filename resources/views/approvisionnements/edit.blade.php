<!-- resources/views/approvisionnements/edit.blade.php -->
@extends('layouts.app')

@section('content')
<div class="container mx-auto p-6 bg-white rounded-lg shadow">
    <h1 class="text-2xl font-bold mb-6 text-gray-800">Modifier l'Approvisionnement</h1>
    ...
    <form action="{{ route('approvisionnements.update',  $approvisionnement->id) }}" method="POST">
            ...
            <select name="produit_id" id="produit_id" class="w-full px-4 py-2 border rounded-lg" required>
                @foreach ($produits as $produit)
                    <option value="{{ $produit->id }}" 
                            {{ $approvisionnement->produit_id == $produit->id ? 'selected' : '' }}>
                        {{ $produit->nom }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="number" name="quantité_fournie" id="quantité_fournie" 
                   ...
                   value="{{ $approvisionnement->quantité_fournie }}" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="number" step="0.01" name="prix_unitaire" id="prix_unitaire" 
                   class="w-full px-4 py-2 border rounded-lg" 
                   value="{{ $approvisionnement->prix_unitaire }}" required>
        </div>

        <div class="mb-4">
            <label for="date_livraison" class="block text-gray-700 font-medium mb-2">Date d'Approvisionnement</label>
            <input type="date" name="date_livraison" id="date_approvisionnement" 
                   class="w-full px-4 py-2 border rounded-lg" 
                   value="{{ $approvisionnement->date_livraison }}" required>
        </div>
            ...
            <select name="fournisseur_id" id="fournisseur_id" class="w-full px-4 py-2 border rounded-lg" required>
                @foreach ($fournisseurs as $fournisseur)
                    <option value="{{ $fournisseur->id }}" 
                            {{ $approvisionnement->fournisseur_id == $fournisseur->id ? 'selected' : '' }}>
                        {{ $fournisseur->nom }}
                    </option>
                @endforeach
            </select>
